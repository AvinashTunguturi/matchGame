...
import DisplayRandomImage from './components/DisplayRandomImage'
...
const sizeOfImageList = imagesList.length
...
const initialState = {
  ...
  imageDetailsToDisplay: imagesList[0],
}
...
class App extends Component {
        ...
        <ul className="score-time-container">
          <li>
            <p className="header-text">
              Score: <span className="number-text">{count}</span>
            </p>
          </li>
          <li className="time-container">
            ...
            <li className="counter-container">
              <p className="number-text">{timerCount} sec</p>
            </li>
          </li>
        </ul>
      </nav>
  ...
  changeDisplayPicture = thumbNailId => {
    const {imageDetailsToDisplay} = this.state
    const {id} = imageDetailsToDisplay

    const getRandomNumber = Math.floor(Math.random() * sizeOfImageList)
    const renderAImageFromImageList = imagesList[getRandomNumber]

    if (id === thumbNailId) {
      this.setState(prevState => ({
        imageDetailsToDisplay: renderAImageFromImageList,
  ...
  renderGameCards = () => {
    ...
    const filteredImageList = imagesList.filter(
      imageItem => imageItem.category === activeTabId,
    )
    ...
    return (
      <div className="category-image-container">
        ...
        <ul className="image-list-container">
          {filteredImageList.map(imageItem => (
            <DisplayRandomImage
              key={imageItem.id}
              imageDetails={imageItem}
              changeDisplayPicture={this.changeDisplayPicture}
            />
          ))}
        </ul>
      </div>
  ...
  renderGame = () => {
    ...
    const {imageUrl} = imageDetailsToDisplay

    return (
      <div className="game-app-container">
        <div className="game-container">
          <div className="">
            <img alt="match" className="display-image" src={imageUrl} />
          </div>
  ...
  renderGameOver = () => {
    ...
    return (
      <div className="game-app-container">
        <div className="game-over-card game-container">
          <img
            alt="trophy"
            className="trophy"
  ...
  render() {
    ...
    return (
      <div className="app-container">
        {this.navBar()}
        {isGameOver ? this.renderGameOver() : this.renderGame()}
      </div>
    )
  }
}
